...
const config = {
    ...
    // Geo-targeting configuration - UPDATED
    geoConfig: {
        ...
        specificRegions: ['EU'] // Can specify 'EU' or other regions
    },
...
// Check geo-targeting restrictions - UPDATED
function checkGeoTargeting(geoData) {
    ...
    if (!geoData || !geoData.country || geoData.country === 'Unknown') {
        console.log('Geo-targeting: Unknown country, defaulting to not show banner');
        return false;
    }

    // Check EU-only mode first
    if (config.geoConfig.euOnly) {
        const isEU = EU_COUNTRIES.includes(geoData.country);
        console.log(`Geo-targeting: EU-only mode, country ${geoData.country} is ${isEU ? 'in EU' : 'not in EU'}`);
        return isEU;
    }
    ...
    if (config.geoConfig.specificRegions.length > 0) {
        if (config.geoConfig.specificRegions.includes('EU') && 
            ...
            console.log(`Geo-targeting: Country ${geoData.country} is in specified EU region`);
            return true;
        }
        const isInSpecificRegion = config.geoConfig.specificRegions.includes(geoData.country);
        console.log(`Geo-targeting: Country ${geoData.country} is ${isInSpecificRegion ? '' : 'not '}in specified regions`);
        return isInSpecificRegion;
    }
    ...
    if (config.geoConfig.allowedCountries.length > 0 && 
        ...
        console.log(`Geo-targeting: Country ${geoData.country} is not in allowed list`);
        return false;
    }
    
    if (config.geoConfig.allowedRegions.length > 0 && 
        !config.geoConfig.allowedRegions.includes(geoData.region)) {
        console.log(`Geo-targeting: Region ${geoData.region} is not in allowed list`);
        return false;
    }
    
    if (config.geoConfig.allowedCities.length > 0 && 
        !config.geoConfig.allowedCities.includes(geoData.city)) {
        console.log(`Geo-targeting: City ${geoData.city} is not in allowed list`);
        return false;
    }
    
    // If no restrictions, allow by default
    console.log('Geo-targeting: No restrictions, allowing by default');
    return true;
}
